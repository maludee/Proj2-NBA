...
class bball2(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        yield scrapy.Request(url = url, callback = self.season_links)
    
    # Gets URLs for all of the season links from initial table
    def season_links(self,response):
        season_years = ("2016", "2015", "2014", "2013", "2012", "2011")
        for i in season_years:
            url_stub = "NBA_" + i + ".html"
            url = 'http://www.basketball-reference.com/leagues/' + url_stub
            season = i
            print season
